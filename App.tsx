...
function App() {
  ...
  const handleGenerate = useCallback(async () => {
    ...
    try {
      const result: RawScenario[] = await generateTestScenarios(userInput);
  ...
  return (
    <div className="min-h-screen bg-slate-900 font-sans p-4 sm:p-6 lg:p-8">
      ...
      <footer className="text-center mt-12 text-slate-500 text-sm">
        ...
        <p className="text-slate-600 mt-1">API Key is managed via environment variables for security.</p>
      </footer>
    </div>
  );
}
